...
from models.build_yolact import Yolact
from utils.augmentations import FastBaseTransform
from utils.functions import MovingAverage, ProgressBar
from dataset.config import update_config
...
parser.add_argument('--traditional_nms', default=False,
                    action='store_true', help='Whether to use traditional nms.')
parser.add_argument('--hide_mask', default=False,
                    action='store_true', help='Whether to display masks')
parser.add_argument('--hide_bbox', default=False,
                    action='store_true', help='Whether to display bboxes')
parser.add_argument('--hide_score', default=False,
                    action='store_true', help='Whether to display scores')
parser.add_argument('--cutout', default=False,
                    action='store_true', help='Whether to cut out each object')
...
parser.add_argument('--image', default=None, type=str,
                    help='The folder of images for detecting.')
...
strs = os.path.basename(args.trained_model).split('_')
print('trained model: ', strs)
...
with torch.no_grad():
    ...
    net = Yolact()
    net.load_weights(args.trained_model, cuda)
    net.eval()
    print('Model loaded.\n')
    ...
    # detect videos
        ...
        while True:
            ...
            if res:
                ...
                nms_outs = NMS(net_outs, args.traditional_nms)
                results = after_nms(
                    nms_outs, img_h, img_w, crop_masks=not args.no_crop, visual_thre=args.visual_thre)
                ...
                frame_numpy = draw_img(results, frame, 'aa', args, fps=fps)
                ...
                print(
                    f'\rDetecting: {repr(progress_bar)} {i + 1} / {num_frames} ({progress:.2f}%) {fps:.2f} fps', end='')
            else:
                break
